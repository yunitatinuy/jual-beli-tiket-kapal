...
<div class="ms-10 mt-8 me-10 flex flex-col flex-grow">
    <span class="flex items-center p-2 text-gray-900 rounded-lg group">
        <img src="/img/icons/moneybag.png" class="flex-shrink-0 w-8 h-8 text-gray-500" aria-hidden="true" fill="currentColor">
        <span class="flex-1 ms-4 whitespace-nowrap text-3xl font-bold">Data Harga</span>
    </span>
        ...
        <div class="container block max-w-2xl overflow-hidden bg-white border border-gray-200 rounded-lg shadow mt-5">
            <form wire:submit="store">
                <div class="bg-[#ff9853be] p-6">
                    <span class="font-inter font-semibold text-xl ">Formulir Harga</span>
                </div>
                ...
                <div class="flex p-6 mb-5">
                    <div class="relative z-0 w-full group">
                        <input type="text" name="Harga" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " wire:model="Harga" required />
                        <label for="" class="peer-focus:font-medium absolute font-inter text-sm text-gray-600 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4    rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Harga</label>
                        @error('Harga')
                        <span class="text-red-500 text-xs mt-3 block">{{$message}}</span>
                        @enderror
                    </div>
...
<div class="flex flex-wrap justify-between ">
    ...
    {{-- Pagination --}}
    <div class="mt-2">
        {{ $dataHarga->links('vendor.pagination.tailwind') }}
    </div>
</div>
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-secondary-200 bg-secondary-50 text-neutral-800">
                            ...
                            <td class="whitespace-nowrap px-6 py-4">
                                ...
                                <!-- Edit -->
                                <button wire:click="edit({{$harga->ID_Harga}})" class="flex-none p-2.5 bg-sky-500 rounded-xl hover:rounded-3xl hover:bg-sky-600 transition-all duration-300 text-white" type="button">
                                ...
                                <!-- Delete -->
                                <button onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?') || event.stopImmediatePropagation()" wire:click="delete({{$harga->ID_Harga}})" class="flex-none p-2.5 bg-red-500 rounded-xl hover:rounded-3xl hover:bg-red-600 transition-all duration-300 text-white">
...
{{-- <div class="mt-2 mb-14">
    {{ $dataHarga->links(data: ['scrollTo' => false]) }}
</div> --}}