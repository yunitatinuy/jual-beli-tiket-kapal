...
<div class="flex items-center justify-center min-h-screen p-5 min-w-screen">
    <div class="w-full p-8 text-center text-gray-800 transition duration-200 shadow-xl bg-slate-100 hover:scale-105 hover:ease-out lg:max-w-3xl rounded-3xl lg:p-12">
        <h3 class="text-3xl">Terima Kasih sudah registrasi di Anang Ferry!</h3>
        <div class="flex justify-center">
            <svg class="w-1/4 p-4 h-1/4" viewBox="0 0 50 21" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="p-4 mb-4 text-sm text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400" role="alert">
            <span class="font-inter">Link verifikasi sudah dikirimkan ke Email Anda!</span>
        </div>
        @endif

        <p class="p-3 font-inter">Kami sangat senang Anda disini. Ayo verifikasi Email Anda: {{ auth()->user()->email }}</p>
        <button class="px-2 py-2 text-white bg-blue-600 rounded font-inter" href="{{ route('verification.send') }}" style="cursor: pointer" onclick="event.preventDefault(); document.getElementById('verify-form').submit()">
            <span class="flex flex-wrap">Kirim Email Verifikasi</span></button>
